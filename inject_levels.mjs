...
const newLevels = JSON.parse(fs.readFileSync('new_32_levels.json', 'utf8'));
...
// Find the end of LEVELS
let parts = content.split('];\n\n// ─── ARROW VISUALS');
if (parts.length === 2) {
  content = parts[0] + ',\n' + levelsStr + '];\n\n// ─── ARROW VISUALS' + parts[1];
}
...
content = content.replace(/const CHAPTERS = \[[\s\S]*?\];/, `const CHAPTERS = [\n${newChapters}\n];`);
content = content.replace(/All 60 levels complete/g, "All 140 levels complete");
content = content.replace(/All 108 levels complete/g, "All 140 levels complete");
content = content.replace(/localStorage\.getItem\('chromaticProgress'\)/g, "localStorage.getItem('chromaticProgress_v4')");
content = content.replace(/localStorage\.setItem\('chromaticProgress'/g, "localStorage.setItem('chromaticProgress_v4'");
