...
function checkMatch(sourceTile, conn, targetTile) {
  ...
  if (conn.dir === "SHELF_UP" || conn.dir === "SHELF_DOWN") {
    return targetTile.type === "STAIRS" && targetTile.stairsDir === (conn.dir === "SHELF_UP" ? "DOWN" : "UP") && targetTile.outer === conn.color;
  }
...
function parseTile(str) {
  if (!str || str === "EMPTY") {
    return { type: "EMPTY", outer: null, connections: [], inPorts: [], id: str || "EMPTY" };
  }
  ...
  if (str.startsWith("OUT:")) {
    ...
    const connections = (innerPart || "").split(",").filter(Boolean).map(p => {
      const parts = p.split(":");
  ...
  if (str.startsWith("TRIGGER:")) {
    const parts = str.split(":"); // TRIGGER:reqColor:targetCell
    return { type: "TRIGGER", reqColor: parts[1], targetCell: parts[2], outer: null, connections: [], inPorts: [], id: str };
  }
  if (str.startsWith("STAIRS:")) {
    const [header, innerPart] = str.split("|");
    const parts = header.split(":"); // STAIRS:dir:outer
    const connections = (innerPart || "").split(",").filter(Boolean).map(p => {
      const [c, d, dist] = p.split(":");
